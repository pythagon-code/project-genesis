...
class ModelOptimizer[ModelType]:
    def __init__(
        ...
        self._frozen_containers = model_containers
        self._dirty_containers = model_opt_containers

        self._model_state = MMapObject(model_filename, self._dirty_containers[0][0].state_dict())
        self._opt_state = MMapObject(optim_filename, self._dirty_containers[0][1].state_dict())


    def load_frozen(self) -> ModelType:
        assert len(self._frozen_containers) > 0
        model = self._frozen_containers.pop()
        model.load_state_dict(self._model_state.load())
        return model


    def load_dirty(self) -> tuple[ModelType, optim.Optimizer]:
        assert len(self._dirty_containers) > 0
        model, opt = self._dirty_containers.pop()
        model.load_state_dict(self._model_state.load())
    ...
    def unload_frozen(self, model: ModelType) -> None:
        self._frozen_containers.append(model)
    ...
    def unload_dirty(self, model: ModelType, opt: optim.Optimizer) -> None:
        ...
        self._dirty_containers.append((model, opt))
