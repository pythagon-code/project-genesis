...
def test_split_mutualism():
    mid_actors = [FNN([4, 32, 32, 4], output=True).to(device) for _ in range(3)]
    mid_actors_target = [FNN([4, 32, 32, 4], output=True).to(device) for _ in range(3)]
    for i in range(3):
...
def test_double_mutualism():
    mid_actors = [FNN([4, 32, 32, 8], output=True).to(device) for _ in range(3)]
    mid_actors_target = [FNN([4, 32, 32, 8], output=True).to(device) for _ in range(3)]
    for i in range(3):
        mid_actors_target[i].load_state_dict(mid_actors[i].state_dict())
